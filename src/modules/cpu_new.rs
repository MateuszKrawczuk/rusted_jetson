...
mod tests {
    ...
    #[test]
    #[ignore = "Requires implementation - failing test for CPU core frequency from sysfs"]
    fn test_read_cpu_core_frequency() {
        ...
        let core0_path = Path::new("/sys/devices/system/cpu/cpu0/cpufreq/scaling_cur_freq");
        let core0_freq = if core0_path.exists() {
            if let Ok(content) = fs::read_to_string(&core0_path) {
    ...
    #[test]
    #[ignore = "Requires implementation - failing test for CPU core utilization calculation"]
    fn test_cpu_core_utilization_calculation() {
        // Test CPU core utilization calculation using all 7 fields from /proc/stat
        // jtop uses: user, nice, system, idle, iowait, irq, softirq

        // Current implementation uses only 4 fields: user, nice, system, idle
        // Should add: iowait, irq, softirq for accuracy
